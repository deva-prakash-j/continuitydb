...
const root = mkdtempSync(join(tmpdir(), "context-vault-quality-"));
...
try {
  ...
  for (const item of fixture.queries) {
    ...
    const hit = results.some((result) => result.id === ids.get(item.expected));
    const stats = byKind.get(item.kind) || { total: 0, hits: 0 };
    stats.total += 1;
    stats.hits += hit ? 1 : 0;
    byKind.set(item.kind, stats);
    if (!hit) failures.push({ kind: item.kind, query: item.query, expected: item.expected });
  }
  ...
  console.log(JSON.stringify({
    mode: "SQLite FTS5 lexical baseline",
