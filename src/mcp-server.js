...
const engine = new HybridEngine(vault, createEmbedderFromEnv());
const server = new McpServer({ name: "continuitydb", version: "0.3.0" });
...
const capturePolicy = new CapturePolicy(loadCapturePolicy(process.env.CONTINUITYDB_CAPTURE_POLICY_FILE || null));
...
server.registerTool(
  ...
  async (input) => response(await engine.search({
    ...input,
...
server.registerTool(
  ...
  async (input) => response(await engine.contextPack({
    ...input,
    tenant_id: identity.tenant_id,
    owner_id: identity.owner_id,
    allowed_projects: allowedProjects,
